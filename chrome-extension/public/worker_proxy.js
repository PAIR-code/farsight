...
(function () {
  ...
  let runtimeMessageHandlerSet = false;

  console.log(location.origin, EXTENSION_ORIGIN);
  if (location.origin == EXTENSION_ORIGIN) {
    ...
    // }
    if (runtimeMessageHandlerSet) {
      // chrome.runtime.onMessage.addListener(backgroundPageMessageHandler);
    } else {
      if (window) {
        window.addEventListener('message', extensionProxyMessageHandler);
        chrome.runtime.onMessage.addListener(backgroundPageMessageHandler);
        runtimeMessageHandlerSet = true;
      }
    }
  } else {
    ...
    function loadFrameAndFlush() {
      ...
      proxyFrame.onload = function () {
        ...
        const result = chrome.runtime
          .sendMessage(MSG_GET_TOKEN)
          .then((token) => {
            console.log('runtime message callback!!');
            if (typeof token != 'string') {
              console.warn(
